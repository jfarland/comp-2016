#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-#
# BigDEAL Forecasting Competiion 2016 - Tao Hong's Energy Analytics Course
#
# Prepare data, train forecasting models, generate forecasts
#
# Author: Jonathan Farland <jonfarland@gmail.com>
#
# Copywright March 2016
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-#
...
setwd("/home/rstudio/projects/comp-2016/data/")
